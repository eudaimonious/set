...
class ViewController: UIViewController {
    ...
    private func dealCards(numberOfCards: Int) {
        let cardsToDeal = deck.nextCards(numberOfCards: numberOfCards)
        for card in cardsToDeal {
            if let button = findEmptyButton() {
                drawCard(button, card)
            }
        }
    }

    private func findEmptyButton() -> UIButton? {
        return cardButtons.filter { $0.attributedTitle(for: UIControlState.normal) == nil }.first
    }
